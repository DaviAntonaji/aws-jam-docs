...
import {
  IAMClient,
  UpdateAssumeRolePolicyCommand, // <-- vem do client-iam (e não do client-eks)
  ...
  if (api_path == "/createAccessEntry") {
    try {
      ...
      const input = {
        ...
        // se no schema vierem opcionais, você pode incluir:
        // kubernetesGroups, username, type, tags
      };
      ...
      } else reason = "error occurred in API call ${api_path}";
      body = { error: reason };
      response_code = 400;
      response_body = { "application/json": { body: JSON.stringify(body) } };
    }
    ...
    try {
      ...
      response_code = 200;
      response_body = {
        "application/json": { body: "Successfully deleted the access entry" },
      };
    } catch (error) {
      if (error instanceof Error) {
        reason = error.message + "API Call - ${api_path}";
    ...
    try {
      // Extrair o roleArn do body (array de {name, value})
      const props =
        event?.["requestBody"]?.["content"]?.["application/json"]?.[
          "properties"
        ] || [];
      let roleArn;
      props.forEach((p) => {
        if (p?.name === "roleArn") roleArn = p?.value;
      });

      if (!roleArn) {
        body = { error: "Missing required field: roleArn" };
        response_code = 400;
        response_body = { "application/json": { body: JSON.stringify(body) } };
      } else {
        // Extrair Account ID do próprio ARN do role
        // arn:aws:iam::<ACCOUNT_ID>:role/<role-name>
        const m = roleArn.match(/^arn:aws:iam::(\d{12}):role\/.+$/);
        const accountId = m ? m[1] : undefined;
        ...
        const input = {
          RoleName: roleArn.split("/").slice(1).join("/"), // suporta roles com path
          PolicyDocument: JSON.stringify(assumeRolePolicyDocument),
        };

        const cmd = new UpdateAssumeRolePolicyCommand(input);
      ...
      const reason =
        error instanceof Error
          ? error.message + " API Call - /updateRolePodIdentity"
          : "error occurred in API call /updateRolePodIdentity";
      body = { error: reason };
      response_code = 400;
      response_body = { "application/json": { body: JSON.stringify(body) } };
    }
